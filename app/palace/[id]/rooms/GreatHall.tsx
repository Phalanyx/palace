...
export function GreatHall({ objects = [], activeObjectIdx = -1, onCloseObject }: { objects?: any[]; activeObjectIdx?: number; onCloseObject?: () => void }) {
  ...
  return (
    <>
      ...
      {objects.slice(0, 5).map((obj, i) => (
        <DynamicObject key={obj.id} objectData={obj} position={GREAT_HALL_SLOTS[i]} forceOpen={i === activeObjectIdx} onClose={onCloseObject} />
      ))}
    </>
  );
}
